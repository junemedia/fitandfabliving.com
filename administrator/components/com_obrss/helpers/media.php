<?php
/**
 * @version		$Id: media.php 732 2013-07-22 08:53:07Z tsvn $
...
class MediaHelper
{
	...
	function canUpload( $file, &$err )
	{
		...
		if (!$params->get('upload_extensions')) {
			$components	=	&JComponentHelper::getComponent('com_obrss');
	...
	{
		$xml_file = JPATH_COMPONENT_ADMINISTRATOR.DS.'obrss.xml';
		if (JFile::exists(JPATH_COMPONENT_ADMINISTRATOR.DS.'obrss.xml')) :
			$xml = simplexml_load_file(JPATH_COMPONENT_ADMINISTRATOR.DS.'obrss.xml');
			if ($xml)
			{
				$version = $xml->version;
				if (!$version = & $xml->version) {
					return false;
				}
			} else {
				return false;
