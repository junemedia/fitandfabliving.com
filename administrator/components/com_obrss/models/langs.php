...
$user = & JFactory::getUser();
	...
	jimport('joomla.utilities.array');
class JlordRssModelLangs extends obModel {
	...
	function getrwlanguage() {
		...
		if($add_write != '') {
			JlordCoreModelLangs::save_into_file();
		}
...
/*				    $temp = explode('=', $data[$i]);*/
	...
	function getLanguage() {
		...
		$dir_langs[] = JHTML::_('select.option', '0', '- '.JText::_('Select Language').' -');
		for($i=0;$i<count($dirs);$i++) {
			if(glob($path.DS.$dirs[$i].DS."*".$filter_file."*."."ini")) {
	...
	/*				$files[$i] = JFolder::files( $path.DS.$dirs[$i], 'com_jlord.core.ini' );*/
		...
		for($i=0;$i<count($dirs1);$i++) {
			if($dirs1[$i] !='pdf_fonts') {
				$dir_langs[] = JHTML::_('select.option', $dirs1[$i], JText::_($dirs1[$i]));
			}
		}
		/*			$files = $pageNav->limit - $pageNav->limitstart ;*/
		jimport('joomla.html.pagination');
		$langs[] = JHTML::_('select.option', '0', '- '.JText::_('Select Local Language').' -');
		$langs[] = JHTML::_('select.option', '1', JText::_('Site Language'));
		$langs[] = JHTML::_('select.option', '2', JText::_('Administrator Language'));
		...
		$file_style[] = JHTML::_('select.option', '*', '- '.JText::_('Select File Style').' -');
