...
class SPUpgradeModelCom_sh404sef extends SPUpgradeModelExtension {
    ...
    private function fix($ids = null, $name) {
        ...
        if ($name == 'redirection')
            $destination_table_name = '#__' . 'sh404sef_urls';
        if ($name == 'sh404sef_meta')
            $destination_table_name = '#__' . 'sh404sef_metas';
        $items = Array();
        ...
        foreach ($items as $i => $item) {
            ...
            //Build query
            $query = "REPLACE INTO " . $destination_table_name . " (";
            $columnNames = Array();
            $values = Array();
        ...
        if ($name == 'sh404sef_meta') {
            $destination_db->setQuery('DROP TABLE #__sh404sef_meta');
            if (!$destination_db->query()) {
                $message = '<p>' . $destination_db->getError() . '</p>';
                $factory->writeLog($message);
                $tableLog->note = $message;
                $tableLog->store();
            }
        ...
        if ($name == 'redirection') {
            $destination_db->setQuery('DROP TABLE #__redirection');
            if (!$destination_db->query()) {
                $message = '<p>' . $destination_db->getError() . '</p>';
                $factory->writeLog($message);
                $tableLog->note = $message;
