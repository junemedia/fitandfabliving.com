 ...
 * SPUpgrades View
 */
class SPUpgradeViewExtensions extends JViewLegacy {
    ...
    protected function addToolBar() {
        ...
        JToolBarHelper::title(JText::_('COM_SPUPGRADE_TABLES_TITLE'), 'media.png');
        
        $view = JRequest::setVar('view', JRequest::getCmd('view', 'extensions'));
        SPUpgradeHelper::addSubmenu('extensions');

        if ($canDo->get('core.admin')) {
            $bar = JToolBar::getInstance('toolbar');
            if ($this->dbTestConnection) {
                $bar->appendButton('Confirm', 'COM_SPUPGRADE_CONFIRM_MSG', 'copy', 'COM_SPUPGRADE_TRANSFER', 'extensions.transfer', true);
                JToolBarHelper::divider();
            }            
            JToolBarHelper::preferences('com_spupgrade');
        }
        $bar = JToolBar::getInstance('toolbar');
        $bar->appendButton('Help', 'help', 'JTOOLBAR_HELP', 'http://cyend.com/extensions/extensions/components/documentation/90-how-to-migrate-extensions-from-old-joomla-15', 640, 480);
    }
