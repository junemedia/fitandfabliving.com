...
class ContentViewArticle extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		$this->moreList = $model->moreFrom($item->id,$item->catid);
	...
	protected function _prepareDocument()
	{
		...
		$textImages = json_decode($this->item->images);
		
		if(isset($textImages->image_intro)||!empty($textImages->image_intro))
		{
			$this->document->addCustomTag('<meta property="og:image" content="'.JURI::base().$textImages->image_intro.'" />');
		}
		else if(isset($slideImages[0])||!empty($slideImages[0]))
		{
			$this->document->addCustomTag('<meta property="og:image" content="'.JURI::base().$slideImages[0].'" />');
		}
    ...
    function removeSlideImage($text){
          jimport('leon.dom');
          $html = str_get_html($text);
          foreach($html->find('img[alt]') as $img){
                $img->parent()->innertext = "";    
          }
