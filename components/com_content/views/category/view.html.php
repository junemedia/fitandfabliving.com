...
class ContentViewCategory extends JViewLegacy
{
	...
	protected $childArticles = array();
	
	protected $newItems = array();
	
	protected $categories = array();
	
	protected $lifestyleArticles = array();
	...
	public function display($tpl = null)
	{
		...
		$sliderItems = $model->getArticles($category->id,null,'publish_up DESC',true,0,4,true);		
		$newItems =  array_diff_key($model->getArticles($category->id,null,'whatsnewfeatured DESC,a.publish_up DESC',false,0,8,true),$sliderItems);
		$this->newItems = array_slice($newItems,0,4,true);unset($newItems);
		
		/*if($category->id == 178)
		{
			$this->lifestyleArticles = $model->getArticles(178,null,'publish_up DESC',false,0,13);
			unset($this->lifestyleArticles[0]);
			unset($this->lifestyleArticles[1]);
			unset($this->lifestyleArticles[2]);
		}*/
		
		//Categories shown in menu
		//Health:163, Fitness:162, Beauty:172, Weight Loss:151, Recipes:173, Lifestyle:178		
		/*$allCategory = array(163,162,172,151,173);
		if($category->id == 178)
		{
			$this->categories = $model->getCategories($allCategory);
		}*/
		...
		foreach ($children[$category->id] as $id => $child){
			$this->childArticles[$child->id]['articles'] = array_diff_key($model->getArticles($child->id,null,'publish_up DESC',false,0,16,true),$this->newItems,$sliderItems);
			/*$this->childArticles[$child->id]['featured'] = $model->getArticles($child->id,null,'publish_up DESC',true,0,1);*/
			//echo $this->childArticles[$child->id]['articles']->id;
			$this->childArticles[$child->id]['cat_link'] = JRoute::_(ContentHelperRoute::getCategoryRoute($child->id));
		}
		unset($sliderItems);
