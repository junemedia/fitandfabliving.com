...
function UsersBuildRoute(&$query)
{
	...
	if (empty($items))
	{
		...
		for ($i = 0, $n = count($items); $i < $n; $i++)
		{
			...
			// Check to see if we have found the author menu item.
			if (empty($author) && !empty($items[$i]->query['view']) && ($items[$i]->query['view'] == 'author'))
			{
				$author = $items[$i]->id;
			}
		}
	...
	if (!empty($query['view']))
	{
		switch ($query['view'])
		{
			...
			case 'author':
				...
				if (!empty($query['user_name']) )//&& ($query['user_name'] != $user->username))
				{
					$segments[] = $query['user_name'];
				}
