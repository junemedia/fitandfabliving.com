...
class JModelList extends JModelLegacy
{
	...
	public function getItems()
	{
		...
		// Let's try cache here
        $cacheLife = 1200; // Define the cache life time.
        $cacheDir = JPATH_CACHE . "/leon/";
        $queryStringEncode = base64_encode($this->_db->replacePrefix((string) $query));
        $queryMd5 = $queryStringEncode; 
        $cacheFile = $cacheDir . "query_list_" . md5($queryMd5) . ".cache";
        //echo $queryMd5 . "<br>";
        //echo $queryString . "<br>";        
        
        $result = getCache($cacheFile, $cacheLife);
        // Articles content page listing stick on the same page. Let's remove the cache for now
        $result = false;
        if($result === false){
		    try
		    {
