...
if($poll_type == 0)
	$cssFile = JURI::base(true).'/components/com_sexypolling/generate.css.php?id_poll='.$id_poll.'&module_id='.$module_id;
else
	$cssFile = JURI::base(true).'/components/com_sexypolling/generate.css.php?id_category='.$category_id.'&module_id='.$module_id;
$document->addStyleSheet($cssFile, 'text/css', null, array());
...
if(sizeof($pollings) > 0) {
	foreach ($pollings as $poll_index => $polling_array) {
		...
		echo '<div class="polling_img"><img src="'.JURI::base(true).'/components/com_sexypolling/assets/images/FF-reader-poll.png"/></div>';
