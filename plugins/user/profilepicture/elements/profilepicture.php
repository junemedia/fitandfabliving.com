...
class JFormFieldProfilePicture extends JFormField
{
	...
	protected function getInput()
	{
		...
		$imgPath = JURI::root().'media'.DS.'plg_user_profilepicture'.DS.'images'.DS;
		$selectHead = '<br><div style="clear:both;"><div style="">OR, choose an avatar below:</div><input type="radio" name="user_icon" id="user_icon" value="1" /> <img src="'.$imgPath.'user-icon-1.jpg"/ >
		<input type="radio" name="user_icon" id="user_icon" value="2" /> <img src="'.$imgPath.'user-icon-2.jpg"/ >
  <input type="radio" name="user_icon" id="user_icon" value="3" /> <img src="'.$imgPath.'user-icon-3.jpg"/ ></div>';

		return '<input type="file" name="' . $this->name . '" id="' . $this->id . '"' . ' value=""' . $accept . $disabled . $class . $size
			. $onchange . ' />'.$profilepicture.$remove_pp.$selectHead;
	}
}
