...
class plgUserProfilePicture extends JPlugin
{
	...
	function onContentPrepareData($context, $data)
	{
		...
		if (is_object($data))
		{
            ...
            // Set the default user pic
            if($result == "") $result = "user-icon-1.jpg";
	...
	function onUserAfterSave($data, $isNew, $result, $error)
	{
		...
		$headerImg = JRequest::getVar( 'user_icon', null);
			...
			try
			{
				...
				$query->insert('#__user_profiles')
					...
					->values(
						...
						.' 1');*/
				$query = 'REPLACE INTO #__user_profiles (user_id, profile_key, profile_value, ordering) VALUES ('.$userId.','.$db->quote(plgUserProfilePicture::PROFILE_KEY).','.$db->quote($pp_filename).',1)';
		...
		if($userId && empty($pp_filename))
		{
			...
			{	
				...
				{
					case '1':
						$headerImgName = 'user-icon-1.jpg';
					break;
					case '2':
						$headerImgName = 'user-icon-2.jpg';
					break;
					case '3':
						$headerImgName = 'user-icon-3.jpg';
					break;
					default:break;
				}
				$this->removeProfilePicture($userId);
				...
				try{
					$db = JFactory::getDBO ();
					$sql = 'REPLACE INTO #__user_profiles (user_id, profile_key, profile_value, ordering) VALUES ('.$userId.','.$db->quote(plgUserProfilePicture::PROFILE_KEY).',"'.$headerImgName.'",1)';
					$db->setQuery($sql);
					if (!$db->query())
					{
						throw new Exception($db->getErrorMsg());						
	...
	function removeProfilePicture($userId)
	{
		if ($userId)
		{
			try
			{
				...
				if( !empty($profile_value) && !in_array($profile_value,array('user-icon-1.jpg','user-icon-2.jpg','user-icon-3.jpg'))) 
				{
